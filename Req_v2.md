...
│  │  - Calls Tornado backend for data                                     │  │
...
| Backend Framework | Tornado | Native async, Panel already uses it, parallel SQL queries |
...
from typing_extensions import Literal  # Python 3.7 compatible
...
DEFAULT_ENVIRONMENT = 'local'  # type: Environment
```
...
"""
Action form components using Pydantic v1 for type-safe parameter definitions.
...
NOTE: Uses Pydantic v1 syntax for Python 3.7 compatibility.
"""

import panel as pn
import param
from typing import Callable, Awaitable, Type, get_type_hints, get_origin, get_args, Union, Dict, Any, List, Optional
...
class WidgetFactory:
    ...
    def create_widget(
        ...
        field_info,  # Pydantic v1 ModelField
    ):
        # type: (...) -> pn.widgets.Widget
        """
        ...
        placeholder = extra.get('placeholder', '') if extra else ''
        return pn.widgets.TextInput(
            name=label,
...
class ActionForm(BaseComponent):
    ...
    The on_submit handler receives a validated Pydantic model instance:
        async def _on_load_intraday(self, params: LoadIntradayParams):
            ...
            await self._api.post('/endpoint', params.dict())  # v1: .dict() not .model_dump()
    """
    ...
    def __init__(
        self,
        name,           # type: str
        params_model,   # type: Type[BaseModel]
        on_submit,      # type: Callable[[BaseModel], Awaitable]
        button_type="primary",  # type: str
        collapsed=True,         # type: bool
        **kwargs
    ):
    ...
    def _build_widgets_from_model(self):
        # type: () -> Dict[str, pn.widgets.Widget]
        """Create widgets by introspecting the Pydantic model."""
        widgets = {}
        
        # Pydantic v1: use __fields__ instead of model_fields
        for field_name, field_info in self._params_model.__fields__.items():
    ...
    def get_current_values_dict(self):
        # type: () -> Dict[str, Any]
        """Get current values as a dict."""
        return self.get_current_values().dict()  # v1: .dict() not .model_dump()
```
...
# ═══════════════════════════════════════════════════════════════════════════
# ACTION PARAMETER MODELS - Pydantic v1 models for each action
# ═══════════════════════════════════════════════════════════════════════════
...
class OverrideSpreadParams(BaseModel):
    ...
    instrument: str = Field(
        ...
        placeholder='e.g., BUND',  # Pydantic v1 uses Field kwargs directly
    )
...
class EurGovtTab(BaseTab):
    ...
    async def _on_refresh_prices(self, params: EmptyParams):
        ...
        await self._api.post(f"{self.api_endpoint}/refresh_prices", params.dict())
    
    async def _on_load_intraday(self, params: LoadIntradayParams):
        """Handle Load Intraday action."""
        # params.as_of_time and params.include_cancelled are typed!
        await self._api.post(f"{self.api_endpoint}/load_intraday", params.dict())
    ...
    async def _on_override_spread(self, params: OverrideSpreadParams):
        ...
        await self._api.post(f"{self.api_endpoint}/override_spread", params.dict())
    
    async def _on_export(self, params: ExportParams):
        """Handle Export action."""
        # params.format is Literal['csv', 'xlsx', 'json'] - IDE knows the options!
        await self._api.post(f"{self.api_endpoint}/export", params.dict())
```
...
"""
API client for communicating with Tornado backend.
...
NOTE: Uses Tornado's AsyncHTTPClient for Python 3.7 compatibility.
"""
...
class RiskAPIClient:
    """
    Client for the Risk Dashboard Tornado backend.
    ...
    async def _request(self, method, path, body=None):
        # type: (str, str, Optional[Dict]) -> Dict[str, Any]
        ...
        request = HTTPRequest(
            ...
            request_timeout=120.0,  # Long timeout for big queries
        )
        ...
        try:
            ...
            raise RuntimeError(f"API request failed: {e.code} {e.message}")
    ...
    async def start_execution(self, request):
        ...
        """
        result = await self._request("POST", "/risk/compute", request)
        return result["execution_id"]
    
    async def fetch_tab(self, execution_id, endpoint):
        ...
        """
        path = f"/risk/{execution_id}{endpoint}"
    ...
    async def abort(self, execution_id):
        # type: (str) -> None
        """
        ...
        """
        await self._request("POST", f"/risk/{execution_id}/abort")
...
# Singleton instance
_api_client = None  # type: Optional[RiskAPIClient]
...
### 7.2 Tornado Backend Server

Example backend server using Tornado (skeleton for reference):

```python
# backend/server.py

"""
Tornado backend server for Risk Dashboard.

Handles:
- Execution management
- Data fetching (SQL, files, etc.)
- Business logic (migrated Excel formulas)
- HYDRA storage for replay/debugging
...
# Thread pool for blocking operations (SQL queries, calculations)
executor = ThreadPoolExecutor(max_workers=20)

# In-memory execution store (use HYDRA in production)
executions = {}  # type: Dict[str, Dict[str, Any]]
...
class ComputeHandler(BaseHandler):
    ...
    async def post(self):
        ...
        # Store execution context (use HYDRA in production)
        ...
        self.write_json({"execution_id": execution_id})


class TabDataHandler(BaseHandler):
    """Fetch data for a specific tab."""
    
    @run_on_executor
    def _compute_tab_data(self, execution_id, tab_name, request_params):
        # type: (str, str, Dict) -> Dict[str, Any]
        """
        ...
        return {
            "summary": {
                "notional": random.randint(10000000, 100000000),
                "pnl": random.randint(-500000, 500000),
                "dv01": random.randint(5000, 50000),
                "status": random.choice(["ok", "warning", "breach"]),
            },
            "alerts": [
                {"message": "Example alert", "severity": "warning"},
            ],
            "positions": [
                {"instrument": f"INST_{i}", "notional": random.randint(100000, 1000000)}
                for i in range(100)
            ],
            "risk_by_tenor": [
                {"tenor": t, "dv01": random.randint(1000, 10000)}
                for t in ["1Y", "2Y", "5Y", "10Y", "30Y"]
            ],
            "risk_by_ccy": [
                {"ccy": c, "dv01": random.randint(5000, 20000)}
                for c in ["EUR", "USD", "GBP"]
            ],
        }
    
    async def get(self, execution_id, tab_name):
        if execution_id not in executions:
            self.set_status(404)
            self.write_json({"error": "Execution not found"})
            return
        
        exec_data = executions[execution_id]
        
        # Check if already computed
        if tab_name in exec_data["results"]:
            self.write_json(exec_data["results"][tab_name])
            return
        
        # Compute in thread pool
        result = await self._compute_tab_data(
            execution_id, 
            tab_name, 
            exec_data["request"]
        )
        ...
        # Cache result
        exec_data["results"][tab_name] = result
...
class AbortHandler(BaseHandler):
    ...
    def post(self, execution_id):
        ...
        self.write_json({"aborted": True})


class ActionHandler(BaseHandler):
    """Handle tab-specific actions."""
    
    async def post(self, tab_name, action):
        params = self.get_json_body()
        
        # Route to appropriate handler
        # In production, you'd have action-specific logic here
        
        self.write_json({"status": "ok", "action": action, "params": params})


def make_app():
    return tornado.web.Application([
        (r"/health", HealthHandler),
        (r"/risk/compute", ComputeHandler),
        (r"/risk/([^/]+)/abort", AbortHandler),
        (r"/risk/([^/]+)/([^/]+)", TabDataHandler),  # /risk/{exec_id}/{tab_name}
        (r"/([^/]+)/([^/]+)", ActionHandler),  # /{tab_name}/{action}
    ])
...
if __name__ == "__main__":
    ...
    print(f"Tornado backend listening on port {args.port}")
    tornado.ioloop.IOLoop.current().start()
```

**Key points:**

- `@run_on_executor` offloads blocking work to thread pool
- Parallel SQL queries can be done with `asyncio.gather` or executor
- Results cached per execution_id for consistency
- Replace fake data generation with real DB queries
...
│                                                                             │
│  HTTP GET → Tornado backend → SQL queries + calculations → JSON response   │
└─────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────────┐
...
### 12.1 Panel Latency: Red Flags to Avoid

Panel uses websockets to push updates to the browser. Latency comes from:
1. Blocking the main thread (event loop can't process other work)
2. Too many widget updates (each triggers a render cycle)
3. Large data transfers (serialization + network)

**Red Flag Checklist — Review Before Every PR:**

| Pattern | Problem | Fix |
|---------|---------|-----|
| `pd.read_sql()` in callback | Blocks event loop | `await run_in_executor(None, fetch)` |
| Multiple widget updates in sequence | N render cycles | `param.parameterized.batch_call_watchers()` |
| Watching `value` on TextInput | Fires every keystroke | Watch `value_throttled` instead |
| All tabs in `pn.Tabs()` | All render on load | Use `defer_load=True` or `dynamic=True` |
| Creating widgets in `__panel__()` | Recreated every render | Create in `__init__`, reuse |
| Pushing 100k+ row DataFrame | Huge serialization | Let Perspective handle it (virtual scroll) |
| `time.sleep()` anywhere | Blocks everything | Use `await asyncio.sleep()` |

### 12.2 Async Discipline
...
# ❌ WRONG: Blocking SQL call
def load_data(self):
    df = pd.read_sql(query, connection)
    self.grid.object = df

# ✅ RIGHT: Async with executor for blocking operations
async def load_data(self):
    loop = asyncio.get_event_loop()
    df = await loop.run_in_executor(None, lambda: pd.read_sql(query, connection))
    self.grid.object = df
```

### 12.3 Batching Updates

```python
# ═══════════════════════════════════════════════════════════════════════════
# RULE: Batch related updates to minimize render cycles
# ═══════════════════════════════════════════════════════════════════════════

import param

# ❌ WRONG: 5 separate render cycles
self.card1.value = data['a']
self.card2.value = data['b']
self.card3.value = data['c']
self.grid.object = df
self.status.object = "Done"

# ✅ RIGHT: 1 render cycle
with param.parameterized.batch_call_watchers(
    self.card1, self.card2, self.card3, self.grid, self.status
):
    self.card1.value = data['a']
    self.card2.value = data['b']
    self.card3.value = data['c']
...
### 12.4 Lazy Tab Loading

```python
# ═══════════════════════════════════════════════════════════════════════════
# RULE: Don't render tabs until they're viewed
# ═══════════════════════════════════════════════════════════════════════════

# ❌ WRONG: All 50 tabs render on startup
tabs = pn.Tabs(*[(t.name, t) for t in all_tabs])

# ✅ RIGHT: Only active tab renders
tabs = pn.Tabs(
    *[(t.name, pn.panel(t, defer_load=True)) for t in all_tabs],
    dynamic=True,
)
```
...
```python
# ═══════════════════════════════════════════════════════════════════════════
# RULE: Create widgets once, update their values
# ═══════════════════════════════════════════════════════════════════════════

class MyComponent(BaseComponent):
    
    # ❌ WRONG: Creates new widgets every render
    def __panel__(self):
        return pn.Column(
            pn.pane.Markdown(f"# {self.title}"),  # New object each time!
            pn.widgets.Button(name="Click"),       # New button each time!
        )
    ...
    def __init__(self, **kwargs):
        ...
        self._layout = pn.Column(self._title_pane, self._button)
    
    def __panel__(self):
        self._title_pane.object = f"# {self.title}"  # Update existing
        return self._layout  # Return same layout object
```

### 12.6 Perspective-Specific Performance

Perspective is designed for large datasets. Let it do its job:

```python
# ═══════════════════════════════════════════════════════════════════════════
# Perspective handles 100k+ rows efficiently via virtual scrolling
# ═══════════════════════════════════════════════════════════════════════════

# ✅ FINE: Perspective will virtual-scroll this
perspective_pane = pn.pane.Perspective(
    df_with_100k_rows,
    sizing_mode='stretch_both',
    # Filtering, sorting, grouping happen CLIENT-SIDE
    # Only visible rows are rendered
)

# ❌ AVOID: Pre-filtering in Python when Perspective can do it
filtered_df = df[df['status'] == 'active']  # Don't do this
perspective_pane.object = filtered_df       # Let user filter in Perspective
```
...
### 13.1 How Panel Works (Unlike React)

Panel is **not** a static file server. The UI logic runs on the server, not in the browser.

```
┌─────────────────────────────────────────────────────────────────────────┐
│  TRADITIONAL WEB APP (React)                                             │
│                                                                          │
│  Browser runs JavaScript (UI logic)  ←──HTTP──→  Server (just files)    │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
...
│  PANEL                                                                   │
│                                                                          │
│  Browser (thin client, just renders) ←─websocket─→ Server (Python runs) │
│                                                                          │
│  Your widgets, state, button handlers — all live on the server.         │
│  Browser just shows what Python tells it to show.                       │
└─────────────────────────────────────────────────────────────────────────┘
```

### 13.2 Session Model

Each user gets their own **hermetic session**. Sessions are completely isolated.
...
│   │  - Grid data    │  │  - Grid data    │  │  - Grid data    │         │
│   │  - Form values  │  │  - Form values  │  │  - Form values  │         │
│   │  - Tab state    │  │  - Tab state    │  │  - Tab state    │         │
│   │                 │  │                 │  │                 │         │
│   │  ~50-80MB       │  │  ~50-80MB       │  │  ~50-80MB       │         │
│   └────────┬────────┘  └────────┬────────┘  └────────┬────────┘         │
│            │                    │                    │                   │
...
- Completely isolated (no shared state between sessions)
- Lives as long as websocket is connected
- Dies when browser closes or network drops
- Cleaned up on daily server restart
- ~50-80MB per session → ~100 users per 8GB server
...
└─────────────────────────────────────────────────────────────────────────┘
  ...
  Traders open browser to: http://rpm.internal
...
┌─────────────────────────────────────────────────────────────────────────┐
│                    LOCAL FALLBACK (Emergency)                            │
└─────────────────────────────────────────────────────────────────────────┘

  When remote servers are down or overloaded:

  [Trader's Laptop]
  │
  ├── Runs: python run_ui.py --env prod --local-ui
  │
  ├── Panel server starts on localhost:5006
  │
  └── Browser opens to localhost:5006
                │
                │ HTTP (over network)
                ▼
      ┌──────────────────────────┐
  ...
  Same data, same calculations. Just UI runs locally.
...
"""
Deployment configuration for RPM.

UI_MODE:
- 'remote': Panel on server (default for production)
- 'local': Panel on user's laptop (fallback, development)

BACKEND_ENV:
- Where to fetch data from (always remote except for local dev)
"""

BACKENDS = {
    'local': 'http://localhost:8000',
    'dev': 'http://dev-risk-api.internal:8000',
    'uat': 'http://uat-risk-api.internal:8000',
    'prod': 'http://prod-risk-api.internal:8000',
}

REMOTE_UI = {
    'dev': 'http://dev-rpm.internal:5006',
    'uat': 'http://uat-rpm.internal:5006',
    'prod': 'http://rpm.internal:5006',
}
```
...
These can be changed via the Settings tab without restart.
"""

from dataclasses import dataclass
from typing import Optional
from config.deployment import BACKENDS
...
class Settings:
    ...
    backend_env: str = 'prod'  # local, dev, uat, prod
    
    @property
    def backend_url(self):
        # type: () -> str
        return BACKENDS[self.backend_env]
    ...
    def colour(self):
        ...
        return colours[self.backend_env]
    
    @property
    def debug(self):
        # type: () -> bool
        return self.backend_env in ('local', 'dev')
...
def update_settings(backend_env=None):
    # type: (Optional[str]) -> None
    global _settings
    if backend_env is not None:
        _settings.backend_env = backend_env
```
...
"""
Start RPM (Risk Position Manager).

Usage:
    # Production (opens browser to remote Panel server)
    python run_ui.py --env prod
    
    # Local fallback (runs Panel locally, connects to prod backend)
    python run_ui.py --env prod --local-ui
    
    # Development (local Panel, local backend)
    python run_ui.py --env local --local-ui

The --local-ui flag is the "break glass" option when servers are down.
"""
...
def main():
    parser = argparse.ArgumentParser(description='RPM - Risk Position Manager')
    parser.add_argument('--env', default='prod', 
                        choices=['local', 'dev', 'uat', 'prod'],
                        help='Backend environment')
    parser.add_argument('--local-ui', action='store_true',
                        help='Run Panel locally instead of using remote server')
    parser.add_argument('--port', type=int, default=5006,
                        help='Port for local Panel server')
    args = parser.parse_args()
    
    if args.local_ui:
        # Run Panel locally
        _start_local_panel(args.env, args.port)
    else:
        # Open browser to remote Panel
        _open_remote_panel(args.env)


def _start_local_panel(env, port):
    # type: (str, int) -> None
    """Start Panel server on localhost."""
    from config.settings import update_settings
    update_settings(backend_env=env)
    
    import panel as pn
    from app import create_app
    
    print(f"=" * 60)
    print(f"  RPM - Local Mode")
    print(f"  Backend: {env}")
    print(f"  URL: http://localhost:{port}")
    print(f"=" * 60)
    
    app = create_app()
    pn.serve(app, port=port, show=True, title="RPM")


def _open_remote_panel(env):
    # type: (str) -> None
    """Open browser to remote Panel server."""
    from config.deployment import REMOTE_UI
    
    if env == 'local':
        print("Error: --env local requires --local-ui flag")
        print("Usage: python run_ui.py --env local --local-ui")
        sys.exit(1)
    
    url = REMOTE_UI[env]
    print(f"Opening RPM: {url}")
    webbrowser.open(url)
...
1. CREATION
   User opens browser → websocket connects → new session created
   All widgets initialized, empty state

2. ACTIVE USE  
   User clicks buttons → Python handlers run → websocket pushes updates
   User loads data → backend returns JSON → grids populate

3. SESSION RESET (user-initiated)
   User clicks "Reset Session" → browser navigates to fresh URL
   Old session abandoned (will be garbage collected)
   New session created with fresh state

4. DISCONNECT
   Browser closes OR network drops → websocket disconnects
   Session orphaned (no longer receiving updates)
   
5. CLEANUP
   Daily server restart → all sessions destroyed
   Memory reclaimed
   Users reconnect → new sessions

What's preserved across sessions:
   - Backend data (in HYDRA/database)
   - Saved overrides (if persisted to backend)
   - User preferences (if stored server-side)

What's NOT preserved:
   - Grid scroll position
   - Tab selection
   - Unsaved form values
   - Client-side Perspective filters
```

### 13.7 Reset Session Component
...
class SessionResetButton(BaseComponent):
    """
    Button to reset the current session.
    
    Creates a fresh session by navigating to the app URL.
    Old session is abandoned and cleaned up on next server restart.
    """
    ...
    def __init__(self):
        ...
        self._button.on_click(self._on_reset)
        
        self._confirm = pn.widgets.Button(
            name="Confirm Reset",
            button_type="danger",
            width=150,
            visible=False,
        )
        self._confirm.on_click(self._on_confirm)
        
        self._cancel = pn.widgets.Button(
            name="Cancel",
            button_type="light", 
            width=100,
            visible=False,
        )
        self._cancel.on_click(self._on_cancel)
        
        self._message = pn.pane.Markdown("", visible=False)
    
    def _on_reset(self, event):
        """Show confirmation."""
        self._message.object = "⚠️ This will clear all loaded data and start fresh."
    ...
    def _on_confirm(self, event):
        """Actually reset - navigate to fresh URL."""
        # This JavaScript will navigate the browser, creating a new session
        # The old session is abandoned
        self._message.object = "Redirecting..."
        pn.state.execute("""
    ...
    def __panel__(self):
        return pn.Column(
            self._button,
            self._message,
            pn.Row(self._confirm, self._cancel),
        )
...
### 13.8 Updated Settings Tab

```python
# tabs/settings.py

"""
Settings tab with environment selection and session management.
"""
...
from config.settings import get_settings, update_settings
from components.base import BaseComponent
from components.session import SessionResetButton


class EnvironmentSelector(BaseComponent):
    """Environment selection dropdown with visual feedback."""
    ...
    def __init__(self):
        ...
        self._env_select = pn.widgets.Select(
            ...
            value=settings.backend_env,
            width=200,
        )
        self._env_select.param.watch(self._on_change, 'value')
        
        self._status = pn.pane.Markdown(self._format_status())
    
    def _on_change(self, event):
        update_settings(backend_env=event.new)
        self._status.object = self._format_status()
    
    def _format_status(self):
        # type: () -> str
        settings = get_settings()
        return f"""
**Current:** <span style="color: {settings.colour}; font-weight: bold;">{settings.display_name}</span>
...
class SettingsTab(BaseTab):
    ...
    name = "⚙️ Settings"
    api_endpoint = None  # Settings tab doesn't call backend
    
    def define_components(self):
        return {
            'environment': EnvironmentSelector(),
            'session_reset': SessionResetButton(),
        }
    
    def define_layout(self, c):
        return pn.Column(
            pn.pane.Markdown("# Settings"),
            pn.pane.Markdown("Configure application settings."),
            pn.Row(
                c['environment'],
                pn.Column(
                    ...
                    c['session_reset'],
                    pn.pane.Markdown(
                        "_Reset clears all loaded data and starts fresh. "
                        "Use if the app is behaving unexpectedly._",
                        styles={'color': '#666', 'font-size': '0.9em'},
                    ),
                ),
            ),
            sizing_mode='stretch_width',
...
### 13.9 What Happens When...

| Scenario | What Happens | User Experience |
|----------|--------------|-----------------|
| **Browser refresh** | New session created | "Load All" to reload data |
| **Browser close + reopen** | New session | Same as refresh |
| **Network blip (5s)** | Websocket reconnects | Brief "Connecting..." then continues |
| **Network down (30s+)** | Session lost | "Connection lost. Please refresh." |
| **Server crash** | Session lost | Refresh → routed to healthy server |
| **Click Reset Session** | Navigate to fresh URL | New session, old abandoned |
| **Daily server restart** | All sessions cleared | Users refresh → new sessions |

### 13.10 The "Traders Screaming" Scenario

```
Remote servers are overloaded / down
              │
              ▼
Support sends email:
  "Please run this command while we fix the servers:
   python run_ui.py --env prod --local-ui"
              │
              ▼
Trader runs command on their laptop
              │
              ▼
Panel starts locally, connects to prod backend
              │
              ▼
Everything works, just running on their laptop
              │
              ▼
No screaming, no Excel regression, job saved
```
...
6. Set up Tornado backend skeleton with mock data
...
- **Python 3.7** — Corporate environment limitation
- **No FastAPI** — Requires Python 3.8+
- **Backend options** — Flask or Tornado only

### Choices

| Choice | Alternatives Considered | Why This Choice |
|--------|------------------------|-----------------|
| UI Framework | Panel + Perspective | Python-native, Perspective integration, websocket handled automatically |
| Backend Framework | **Tornado** (not Flask) | Native async for parallel SQL, Panel already uses it, same event loop |
| Data Grid | Perspective | JPM-created, 100k+ row performance, client-side filtering |
| Validation | **Pydantic v1** (not v2) | v2 requires Python 3.8+; v1 works on 3.7 |
| HTTP Client | tornado.httpclient or httpx | Async support for backend calls |
| Config | No .env files | Explicit configuration, version controlled, no magic |

### Python 3.7 Compatibility Notes

All code must use Python 3.7 compatible syntax:
...
```python
# WRONG (Python 3.8+ built-in)
from typing import Literal

# CORRECT (Python 3.7 - use typing_extensions)
from typing_extensions import Literal
```

```python
# Pydantic v1 syntax (DIFFERENT from v2 shown in examples)
from pydantic import BaseModel, Field

class MyModel(BaseModel):
    name: str = Field(..., description="The name")  # ... means required
    count: int = Field(default=0)
    
    class Config:
        # v1 uses Config class, not model_config dict
        extra = "forbid"

# Accessing fields (v1 API)
model.__fields__      # NOT model.model_fields (v2)
model.dict()          # NOT model.model_dump() (v2)
model.json()          # NOT model.model_dump_json() (v2)
MyModel.schema()      # NOT MyModel.model_json_schema() (v2)
```
